    ...
    public class SelectMethod
    {
        ...
        private IEnumerable<int> GetNumbersFromObject(IEnumerable<object> objects)
        {
            ...
            */
            ...
            var result = objects.OfType<int>().Concat(objects.OfType<string>().Select(str =>
            {
                return int.TryParse(str, out int number) ? number : (int?)null;
            }).Where(nullable => nullable != null).Select(nullable => nullable.Value).OrderBy(n => n));

            return result;
        }

        private IEnumerable<User> GetUserInfoFromString(string usersInfo, string emailDomain)
        {
            /*
            ...
            var result = usersInfo.Split(';').Select(userInfo =>
            {
                var userData = userInfo.Split(',');
                var id = Int32.Parse(userData[0].Trim());
                var name = userData[1].Trim();
                var email = userData[2];

                return new User(id, name, email);
            }).Where(user => user.Email.Contains(emailDomain));
        ...
        private int GetSumOfNestedListsWithNumbers(List<List<int>> numbers)
        {
            ...
            return numbers.SelectMany(list => list).Sum();
        }
        ...
        private IEnumerable<string> GetCarMarkModelCollection(List<string> marks, List<string> models)
        {
            ...
            var result = marks.SelectMany(_ => models, (mark, model) => $"{mark} {model}");
        ...
        public void ObjectWithNumbersAndTryParseStrings()
        {
            ...
            Assert.That(new List<object> { 1, 13, 23, 44 }, Is.EquivalentTo(result));
        }
        ...
        public void UserInfoConvertedToUserObject(string emaildDomain, string userName)
        {
            ...
            Assert.That(userName, Is.EqualTo(result.FirstOrDefault().Name), "The name of the user is not correct");
        }
        ...
        public void SumOfNestedListsNumbers(int sum)
        {
            ...
            Assert.That(sum, Is.EqualTo(result), "The sum is not correct");
        }
        ...
        public void CarMarkModelCollection()
        {
            ...
            Assert.That(StringsCollections.ExpectedMarksModelsCollection, Is.EqualTo(result), "Collection of cars is not correct");
        }
    }
}
